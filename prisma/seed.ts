...
async function seedClubAdmin() {
  ...
  if (!club) {
    console.error("   ❌ Club 'Addis Ababa Runners Club' not found. Skipping club admin seed.");
    return;
  }
  ...
  if (!clubAdminRole) {
    console.error("   ❌ CLUB_ADMIN role not found. Skipping club admin seed.");
    return;
  }
...
async function main() {
  ...
  await safeSeed("Club Admin", seedClubAdmin);
